...
import itertools
# 푸리에에서 Constant-Q로 변환해주는 소스

spt = []
ins = []
n = 0
for instrument, note in itertools.product(range(128), range(10)):
    y, sr = librosa.load('dm.wav', sr=None, offset=n, duration=2.0)  # n초지점부터 2초까지만 데이터를 읽어옵니다.
    n += 2
...
for note in range(46):
    y, sr = librosa.load('dm.wav', sr=None, offset=n, duration=2.0)
    n += 2
