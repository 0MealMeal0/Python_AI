...
# Time Signatures
time_imgs = {
    "common": [cv2.imread(time, 0) for time in [f"resources/template/time/common.jpg"]],
...
#global result_pitches # 인식한 음들을 저장할 전역변수

#if __name__ == "__main__":
def anlayze_score(score_location):
    result_pitches = []
    #-------------------------------------------------------------------------------
        ...
        for j in range(len(bars)):
            ...
            for k in range(len(primitives)):
                ...
                if (primitives[k].getPrimitive() == "note"):
                    ...
                    result_pitches.append(primitives[k].getPitch())
                    # 출력된 값이 note이면 음들이 모인 리스트(result_pitches)에 추가
